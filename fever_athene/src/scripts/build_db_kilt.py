...
def get_contents_from_chunk(chunk):
    ...
    for line in chunk:
        ...
        id = utils.normalize(doc['wikipedia_title'])
        wiki_num = doc['wikipedia_id']
        lines = " ".join(doc['text']) # list of paragraph texts

        # documents.append((utils.normalize(doc['id']), doc['text'], doc['lines']))
        documents.append((id, wiki_num, lines))
    return documents


def my_store_contents(data_path, save_path, preprocess, num_workers=None):
    ...
    c.execute("CREATE TABLE documents (id PRIMARY KEY, wikipedia_id, lines);")
    ...
    with open(data_path) as f:
        for piece in tqdm(read_in_chunks(f)):
            pairs = get_contents_from_chunk(piece)
...
def get_contents(filename):
    ...
    with open(filename) as f:
        ...
        for line in f:
            ...
            id = utils.normalize(doc['wikipedia_title'])
            wiki_num = doc['wikipedia_id']
            lines = doc['text'] # list of paragraph texts
            # documents.append((utils.normalize(doc['id']), doc['text'], doc['lines']))
            documents.append((id, wiki_num, lines))
...
# PYTHONPATH=. python3 scripts/build_db_kilt.py ../../KILT/kilt_knowledgesource.json ../../kilt_db.db
